...
local function session_setup(user, pass, google_auth, allowed_users)
--用户登录
	local lan_ip = string.sub(sys.exec("uci get network.lan.ipaddr"),1,7)
	local remote_ip = string.sub(http.getenv("REMOTE_ADDR"),1,7)
	local bip = (string.sub(sys.exec("uci get network.lan.ipaddr"),1,7)) ~= (string.sub(http.getenv("REMOTE_ADDR"),1,7))
	local bauth =(string.sub(luci.sys.exec("uci get google-auth.config.enabled"),1,1))=="1"
	local seckey = sys.exec("uci get google-auth.config.seckey")
	seckey = string.gsub(seckey, "^%s*(.-)%s*$", "%1")
	--seckey="ky3zae3o5rvurrmxpfunjqxwj6fi7dwg"
	--google_auth="778899"
    if(google_auth==nil) then
	    google_auth="123"
	end
	if (bauth and bip) then
		if (string.len(google_auth)>5) then    --内网地址不需要动态码
			local bauth_check = gauth.Check(seckey, google_auth)
			--log.print(bauth_check)
			if(not bauth_check) then
				return nil, nil
			end
		else
		return nil,nil	
...
function dispatch(request)
	...
	if track.sysauth and not ctx.authsession then
		...
		if not (sid and sdat) and authen == "htmlauth" then
			...
			local google_auth = http.getenv("HTTP_AUTH_GOOGLEAUTH")

			if user == nil and pass == nil then
				user = http.formvalue("luci_username")
				pass = http.formvalue("luci_password")
				google_auth = http.formvalue("luci_google_auth")
			end


			sid, sdat = session_setup(user, pass, google_auth, allowed_users)
